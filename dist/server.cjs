...
var initialCmsContent = {
  ...
  valueProp: {
    ...
    videos: [
      {
        id: "vid-1",
        title: "Recorrido Arquitect\xF3nico 3D - Complejo Mis Delirios Ranch",
        videoUrl: "https://assets.mixkit.co/videos/preview/mixkit-flying-over-a-green-mountain-valley-41004-large.mp4",
      ...
      {
        id: "vid-2",
        title: "Bioconstrucci\xF3n Sismorresistente en Guadua Angustifolia",
        videoUrl: "https://assets.mixkit.co/videos/preview/mixkit-pine-trees-in-a-forest-on-a-windy-day-41005-large.mp4",
        posterUrl: "/api/images/bamboo-structure",
        description: "Detalle de ensamble estructural, losa flotante de 40 cm y tecnolog\xEDa bioclim\xE1tica.",
        duration: "2:10"
      },
      {
        id: "vid-3",
        title: "Entorno Campestre, Neblina y Mini-granjas Productivas",
        videoUrl: "https://assets.mixkit.co/videos/preview/mixkit-river-surrounded-by-trees-in-a-forest-41006-large.mp4",
        posterUrl: "/api/images/hero-landscape",
        description: "Microclima templado de 18\xB0C-22\xB0C, huertos familiares y conexi\xF3n con la naturaleza.",
        duration: "1:30"
      }
    ],
...
var CONFIG_FILE = import_path.default.join(process.cwd(), "data", "mariadb-config.json");
var rawEnvHost = process.env.MARIADB_HOST || "";
var cleanEnvHost = sanitizeHost(rawEnvHost);
var defaultConfig = {
...
function loadSavedConfig() {
  ...
  try {
    if (import_fs.default.existsSync(CONFIG_FILE)) {
      saved = JSON.parse(import_fs.default.readFileSync(CONFIG_FILE, "utf-8"));
    }
  } catch (err) {
...
function saveConfig(cfg) {
  ...
  try {
    const dataDir = import_path.default.dirname(CONFIG_FILE);
    if (!import_fs.default.existsSync(dataDir)) {
      import_fs.default.mkdirSync(dataDir, { recursive: true });
    }
    import_fs.default.writeFileSync(CONFIG_FILE, JSON.stringify(currentConfig, null, 2), "utf-8");
  } catch (err) {
...
async function testMariaDbConnection(configOverride) {
  ...
  try {
    ...
    try {
      ...
      if (dbErr.code === "ER_BAD_DB_ERROR" || dbErr.message?.includes("Unknown database")) {
        ...
        try {
          ...
          return {
            success: true,
            message: `\xA1Conexi\xF3n exitosa! La base de datos '${cfg.database}' fue creada autom\xE1ticamente en MariaDB.`,
    ...
    return {
      success: true,
      message: `\xA1Conectado exitosamente a MariaDB! Versi\xF3n: ${rows?.[0]?.version || "OK"}`,
...
async function ensureMariaDbTables() {
...
async function migrateAllToMariaDb(data) {
  ...
  const results = {};
  if (data.content) {
    results.content = await saveMariaDbContent(data.content, data.content.version || 1, "Migraci\xF3n inicial a MariaDB");
  }
  ...
  return {
    success: true,
    message: `\xA1Migraci\xF3n completada con \xE9xito en MariaDB! (${results.lotsCount || 0} lotes, ${results.modelsCount || 0} modelos y contenido global)`,
    details: results
  };
}
initMariaDbPool();
...
var CONTENT_FILE = import_path2.default.join(DATA_DIR, "cms_content.json");
var MODELS_FILE = import_path2.default.join(DATA_DIR, "models.json");
var LOTS_FILE = import_path2.default.join(DATA_DIR, "lots.json");
var LEADS_FILE = import_path2.default.join(DATA_DIR, "leads.json");
var USERS_FILE = import_path2.default.join(DATA_DIR, "users.json");
var VERSIONS_FILE = import_path2.default.join(DATA_DIR, "versions.json");
...
function loadJsonFile(filePath, fallback) {
  ...
  import_fs2.default.writeFileSync(filePath, JSON.stringify(fallback, null, 2), "utf-8");
  return fallback;
}
function saveJsonFile(filePath, data) {
  try {
    const tmpPath = `${filePath}.tmp`;
    import_fs2.default.writeFileSync(tmpPath, JSON.stringify(data, null, 2), "utf-8");
...
async function startServer() {
  ...
  app.use("/api/uploads", import_express.default.static(UPLOADS_DIR));
      ...
      if (obj.startsWith("data:")) {
        try {
          ...
          if (commaIdx !== -1) {
            ...
            const destPath = import_path2.default.join(UPLOADS_DIR, safeFileName);
            import_fs2.default.writeFileSync(destPath, Buffer.from(rawBase64, "base64"));
  ...
  app.post("/api/content", (req, res) => {
    try {
      ...
      res.json({ success: true, data: cmsContent, message: "Contenido actualizado y publicado con \xE9xito" });
      saveMariaDbContent(cmsContent, nextVersion, req.query.note ? String(req.query.note) : void 0).catch((e) => {
  ...
  app.post("/api/sync-all", (req, res) => {
    try {
      ...
      saveMariaDbContent(cmsContent, nextVersion, req.query.note ? String(req.query.note) : void 0).catch(() => {
      });
      ...
      res.json({
        success: true,
        message: "Base de datos sincronizada y persistida en tiempo de ejecuci\xF3n",
        data: {
          content: cmsContent,
          lots: lotsData
        }
  ...
  app.post("/api/models/bulk-save", (req, res) => {
    try {
      ...
      res.json({
        ...
        data: modelsData,
        section: cmsContent.housingModels,
        message: "Cat\xE1logo de modelos y configuraci\xF3n de secci\xF3n sincronizados exitosamente"
      });
    } catch (err) {
      res.status(500).json({ success: false, error: err.message });
    }
